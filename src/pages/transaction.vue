        ...
        <v-card-text class="title">
          ...
          <v-flex d-flex>
            <div>
              <p class="text-left pl-5">
                Транзакция - это группа взаимосвязанных изменений в базе данных,
              ...
              <p class="text-left pl-5">
                В следующем примере рассматривается база данных, в которой содержится информация о продажах недвижимости: лоты (квартира),
                корпуса (дом) и проекты (жилищных комплексах). У каждого из объектов есть статус: в продаже
                или нет. Для обеспечения скорости обработки
                запросов на чтение этот статус хранится в каждой сущности. Если статус
              ...
              <p class="text-left pl-5">
                ...
                данных, чтобы не получилось так, что статус корпуса обновился,
                а связанних с ним лотов - нет. В противном случае может
                оказаться, что проект находится в продаже, корпус в продаже, но при
          ...
          <v-flex d-flex>
            <div>
              ...
              <p class="text-left pl-7">
                - таблица &#9313; "buildings" содержит информацию о корпусах;
              </p>
              <p class="text-left pl-7">
          ...
          <v-flex d-flex>
            <p class="text-left pl-5">
              ...
              таблица lots содержит в себе 4 записи о лотах, лот с id = 2 имеет статус "не актуален";
          ...
          <v-flex d-flex>
            <div>
              <p class="text-left pl-5">
              Можно воспользоваться API и удалить лот с ID = 1.
              </p>
            </div>
          </v-flex>
          ...
          <v-flex d-flex mt-5>
            <div>
              <p class="text-left pl-7 mt-5">
                При вызове корректного API (correct API) вместе с лотом был удален корпус, в котором располагается данный лот, а также проект,
                к которому относится корпус.
                <br/>
                При вызове
                некорректого API (incorrect API) был удален только лот, что нарушило целостность
                базы данных.
                <br/>
                Чтобы вернуть базу данных к исходному состоянию
                необходимо нажать кнопку "return to original state".
              </p>
